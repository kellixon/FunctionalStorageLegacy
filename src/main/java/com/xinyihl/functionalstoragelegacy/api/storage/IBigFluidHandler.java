 ...
 */
public interface IBigFluidHandler extends IFluidHandler, IStorageHandler<BigFluidStack, FluidStorageKey> {
    ...
    default IFluidTankProperties[] getTankProperties() {
        ...
        for (int index = 0; index < count; index++) {
            ...
            final BigFluidStack snapshot = getSnapshot(storageIndex);
            long longCapacity = Math.max(0L, getCapacity(storageIndex));
            final int capacity = longCapacity >= Integer.MAX_VALUE ? Integer.MAX_VALUE : (int) longCapacity;
            properties[index] = new IFluidTankProperties() {
                ...
                public boolean canFillFluidType(FluidStack fluidStack) {
                    return fluidStack != null && handler.supportsFill(storageIndex) && handler.supportsFluid(storageIndex, new BigFluidStack(fluidStack, 1L));
                }

                @Override
                public boolean canDrainFluidType(FluidStack fluidStack) {
                    return fluidStack != null && handler.supportsDrain(storageIndex) && handler.supportsFluid(storageIndex, new BigFluidStack(fluidStack, 1L));
                }
            };
        }
        return properties;
    ...
    default int fill(@Nullable FluidStack resource, boolean doFill) {
        ...
        TransferResult<BigFluidStack, FluidStorageKey> result = fillRouted(request, doFill ? StorageAction.EXECUTE : StorageAction.SIMULATE);
    ...
    default FluidStack drain(@Nullable FluidStack resource, boolean doDrain) {
        ...
        BigFluidStack request = new BigFluidStack(resource, resource.amount);
        TransferResult<BigFluidStack, FluidStorageKey> result = drainRouted(request, doDrain ? StorageAction.EXECUTE : StorageAction.SIMULATE);
        long processed = Math.min(request.getAmount(), Math.max(0L, result.getProcessedAmount()));
    ...
    default FluidStack drain(int maxDrain, boolean doDrain) {
        ...
        TransferResult<BigFluidStack, FluidStorageKey> result = drainRouted(maxDrain, doDrain ? StorageAction.EXECUTE : StorageAction.SIMULATE);
     ...
     */
    @Nonnull
    default TransferResult<BigFluidStack, FluidStorageKey> fillRouted(@Nonnull BigFluidStack request, @Nonnull StorageAction action) {
        ...
        for (int pass = 0; pass < 2 && processedTotal < requested; pass++) {
            for (int index = 0; index < count && processedTotal < requested; index++) {
                ...
                TransferResult<BigFluidStack, FluidStorageKey> result = insert(index, request.withAmount(remaining), action);
     ...
     */
    @Nonnull
    default TransferResult<BigFluidStack, FluidStorageKey> drainRouted(@Nonnull BigFluidStack request, @Nonnull StorageAction action) {
        Objects.requireNonNull(action, "action");
        long requested = request.isEmpty() ? 0L : request.getAmount();
        if (requested == 0L) {
            return new TransferResult<>(0L, BigFluidStack.empty(), action);
