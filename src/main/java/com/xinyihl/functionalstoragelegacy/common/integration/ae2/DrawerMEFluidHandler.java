 ...
 */
public class DrawerMEFluidHandler implements IDrawerMEInventoryHandler<IAEFluidStack>, AE2StorageChangeSource<IAEFluidStack> {
    ...
    private static BigFluidStack requestOf(IAEFluidStack stack) {
        ...
        return definition == null ? BigFluidStack.empty() : new BigFluidStack(definition, stack.getStackSize());
    }
    ...
    public IAEFluidStack injectItems(IAEFluidStack input, Actionable type, IActionSource src) {
        ...
        TransferResult<BigFluidStack, FluidStorageKey> result = fillRouted(request, actionOf(type));
    ...
    public IAEFluidStack extractItems(IAEFluidStack requestStack, Actionable mode, IActionSource src) {
        ...
        TransferResult<BigFluidStack, FluidStorageKey> result = drainRouted(request, actionOf(mode));
    ...
    public boolean canAccept(IAEFluidStack input) {
        BigFluidStack request = requestOf(input);
        return !request.isEmpty() && fillRouted(request.withAmount(1L), StorageAction.SIMULATE).getProcessedAmount() == 1L;
    }
    ...
    private ControllerFluidHandler controller() {
        return handler instanceof ControllerFluidHandler ? (ControllerFluidHandler) handler : null;
    }

    private TransferResult<BigFluidStack, FluidStorageKey> fillRouted(BigFluidStack request, StorageAction action) {
        ControllerFluidHandler controller = controller();
        return controller == null ? handler.fillRouted(request, action) : controller.fillRouted(request, action);
    }

    private TransferResult<BigFluidStack, FluidStorageKey> drainRouted(BigFluidStack request, StorageAction action) {
        ControllerFluidHandler controller = controller();
        return controller == null ? handler.drainRouted(request, action) : controller.drainRouted(request, action);
    }
}
